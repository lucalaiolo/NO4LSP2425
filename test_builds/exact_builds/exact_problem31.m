function [f, gradf, Hessf] = exact_problem31()
% exact_problem31
%
% Computes the Broyden tridiagonal function in R^n (n > 2), 
...
fk = @(x) [
    (3 - 2 * x(1,:)) .* x(1,:) + 1 - 2*x(2,:);
    (3 - 2 .* x(2:end-1,:)) .* x(2:end-1,:) + 1 - x(1:end-2,:) - 2 .* x(3:end,:);
...
gradf_build = [
    fkx(1) * (3 - 4 * x(1)) - fkx(2);
    -2 * fkx(1:end-2) + fkx(2:end-1) .* (3 - 4 .* x(2:end-1)) - fkx(3:end);
    -2 * fkx(end-1) + (3 - 4 * x(end)) * fkx(end);
];
...
d = [
    (3 - 4 * x(1))^2 - 4 * fkx(1) + 1;
    (3 - 4 * x(2:end-1)).^2 - 4 .* fkx(2:end-1) + 5;
    (3 - 4 * x(end))^2 - 4 * fkx(end) + 4;
];

d_plus = -9 + 8 * x(1:end-1) + 4 * x(2:end); 

d_plus_plus = 2 * ones(n - 2, 1);
